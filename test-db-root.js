...
const bniPagePath = path.join(__dirname, "src", "app", "instrutor", "comercial", "apresentacao", "page.tsx");
const mainPagePath = path.join(__dirname, "src", "app", "instrutor", "apresentacao", "page.tsx");

if (!fs.existsSync(bniPagePath) || !fs.existsSync(mainPagePath)) {
  console.error("Paths not found!");
  process.exit(1);
}

const bniContent = fs.readFileSync(bniPagePath, "utf-8");
const mainContent = fs.readFileSync(mainPagePath, "utf-8");

// Let's create the merged page.tsx
// We will build a unified component that has both the BNI states and the Classes states, 
// and renders either BNI cockpit or Class cockpit based on whether `activeBniDeck` is set.

// Let's extract SPEAKER_NOTES and SLIDE_TARGETS_CUMULATIVE from BNI page
const notesStart = bniContent.indexOf("const SPEAKER_NOTES");
const notesEnd = bniContent.indexOf("export default function BniPresentationPage()");
const speakerNotesBlock = bniContent.substring(notesStart, notesEnd);

console.log("Extracted speaker notes block length:", speakerNotesBlock.length);

// Let's write a unified React page.tsx
const unifiedCode = `"use client";
...
export default function UnifiedApresentacaoPage() {
  ...
  const [activeBniDeck, setActiveBniDeck] = useState<"eventos" | "projetos" | "treinamentos" | "consultoria" | null>(null);
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [activeBniDeck, currentSlideIndex]);
  ...
  }, [activeBniDeck, currentSlideIndex, notesOpen]);
  ...
  const bniActiveNote = activeBniDeck ? SPEAKER_NOTES[activeBniDeck][currentSlideIndex + 1] : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={\`px-3 py-0.5 rounded-full text-[9px] font-extrabold border uppercase tracking-wider \${
                activeBniDeck === "eventos"
                  ? "bg-red-500/10 text-red-400 border-red-500/20"
                  : activeBniDeck === "projetos"
                    ? "bg-amber-500/10 text-amber-400 border-amber-500/20"
                    : activeBniDeck === "treinamentos"
                      ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                      : "bg-sky-500/10 text-sky-400 border-sky-500/20"
              }\`}
            >
              {activeBniDeck === "eventos" ? "Eventos" : activeBniDeck === "projetos" ? "SC Fire 1:1" : activeBniDeck === "treinamentos" ? "Treinamentos" : "Consultoria"}
            </span>
          </div>
        ...
        <div className="flex-1 flex overflow-hidden relative z-10">
          <main className="flex-1 flex items-center justify-center relative p-6 overflow-hidden">
            ...
            >
              ...
              {currentSlideIndex === 0 && (
                <div className="w-full h-full flex flex-col items-center justify-center text-center space-y-6 relative">
                  ...
                  <p className="text-gray-400 max-w-2xl leading-relaxed text-sm">
                    {activeBniDeck === "eventos" && "Nascemos em um dos maiores festivais do sul do país. Conheça nosso ecossistema de soluções preventivas baseadas na IN 24."}
                    {activeBniDeck === "projetos" && "Desatamos a burocracia do Corpo de Bombeiros através de engenharia preventiva e projetos técnicos contra incêndio de alto desempenho."}
                    {activeBniDeck === "treinamentos" && "Formação e Reciclagem Técnica de Brigadas Corporativas Orgânicas em estrita conformidade legal com a IN 28 do CBMSC."}
                    {activeBniDeck === "consultoria" && "Blindamos síndicos prediais e proprietários comerciais contra fiscalizações inesperadas de segurança contra incêndio."}
                  </p>
                  ...
                  {activeBniDeck === "treinamentos" && (
                    <div className="presenter-badge-cover mt-4">
                      ...
                      <div className="presenter-badge-info">
                        <h4 className="font-bold text-sm">Sargento BM Murilo Galdino</h4>
                        <p className="text-xs text-gray-400">Especialista de Referência — Divisão de Treinamentos</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-3 gap-6 w-full max-w-[950px] mt-8">
                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] text-center space-y-2">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary mx-auto">
                        {activeBniDeck === "eventos" ? <PartyPopper className="w-5 h-5" /> : activeBniDeck === "projetos" ? <FileCheck2 className="w-5 h-5" /> : activeBniDeck === "treinamentos" ? <Users className="w-5 h-5" /> : <ClipboardCheck className="w-5 h-5" />}
                      </div>
                      <h3 className="text-sm font-bold text-white uppercase">{activeBniDeck === "eventos" ? "EVENTOS (IN 24)" : activeBniDeck === "projetos" ? "PPCI Predial" : activeBniDeck === "treinamentos" ? "Brigadas Orgânicas" : "Pré-Vistoria Diagnóstica"}</h3>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        {activeBniDeck === "eventos" && "Projetos temporários, vistorias estruturais rápidas e Brigadistas sob a IN 24."}
                        {activeBniDeck === "projetos" && "Elaboração de Projetos Preventivos para condomínios e indústrias."}
                        {activeBniDeck === "treinamentos" && "Capacitação obrigatória de colaboradores locais (IN 28) para controle de pânico."}
                        {activeBniDeck === "consultoria" && "Auditoria de conformidade predial sob as 35 Instruções Normativas."}
                      </p>
                    </div>

                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] text-center space-y-2">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary mx-auto">
                        {activeBniDeck === "eventos" ? <FileText className="w-5 h-5" /> : activeBniDeck === "projetos" ? <Award className="w-5 h-5" /> : activeBniDeck === "treinamentos" ? <ShieldCheck className="w-5 h-5" /> : <FileCheck2 className="w-5 h-5" />}
                      </div>
                      <h3 className="text-sm font-bold text-white uppercase">{activeBniDeck === "eventos" ? "PROJETOS" : activeBniDeck === "projetos" ? "Regularização (AVCB)" : activeBniDeck === "treinamentos" ? "Brigadistas Particulares" : "Laudos & Pareceres"}</h3>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        {activeBniDeck === "eventos" && "Elaboração e aprovação de PPCI e Habite-se de edificações prediais."}
                        {activeBniDeck === "projetos" && "Protocolo digital rápido para Habite-se predial e alvarás dos bombeiros."}
                        {activeBniDeck === "treinamentos" && "Formação técnica e reciclagens homologadas para atuação comercial dedicada."}
                        {activeBniDeck === "consultoria" && "Atestados de gás, alarmes de incêndio, portas corta-fogo e SPDA."}
                      </p>
                    </div>

                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] text-center space-y-2">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary mx-auto">
                        {activeBniDeck === "eventos" ? <GraduationCap className="w-5 h-5" /> : activeBniDeck === "projetos" ? <Sliders className="w-5 h-5" /> : activeBniDeck === "treinamentos" ? <Activity className="w-5 h-5" /> : <RefreshCw className="w-5 h-5" />}
                      </div>
                      <h3 className="text-sm font-bold text-white uppercase">{activeBniDeck === "eventos" ? "TREINAMENTOS" : activeBniDeck === "projetos" ? "Alto Desempenho" : activeBniDeck === "treinamentos" ? "Primeiros Socorros" : "Renovação Simplificada"}</h3>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        {activeBniDeck === "eventos" && "Capacitação técnica de brigadas orgânicas internas e primeiros socorros."}
                        {activeBniDeck === "projetos" && "Estudos de fluidodinâmica 3D por supercomputadores (CFD)."}
                        {activeBniDeck === "treinamentos" && "RCP teórico-prático homologado de acordo com a Lei Lucas."}
                        {activeBniDeck === "consultoria" && "Assessoria digital burocrática para licenciamento anual sem multas."}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {currentSlideIndex === 1 && (
                <div className="w-full h-full flex flex-col justify-center space-y-8 relative">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold max-w-fit uppercase tracking-wider">
                    ...
                    <span>{activeBniDeck === "eventos" ? "Força do Setor" : activeBniDeck === "projetos" ? "Segurança Jurídica" : activeBniDeck === "treinamentos" ? "Conformidade Legal" : "Por que Consultoria?"}</span>
                  </div>

                  <h2 className="text-3xl font-extrabold tracking-tight text-white font-display">
                    {activeBniDeck === "eventos" && <>O Impacto Gigante do <span className="text-red-500">Turismo de Eventos em Floripa</span></>}
                    {activeBniDeck === "projetos" && <>Por que Regularizar? <span className="text-amber-500">Riscos e Sanções Inesperadas</span></>}
                    {activeBniDeck === "treinamentos" && <>A Exigência Técnica da <span className="text-emerald-400 font-display">Instrução Normativa 28</span></>}
                    {activeBniDeck === "consultoria" && <>A Auditoria Preventiva: <span className="text-sky-400">Aja Antes da Fiscalização</span></>}
                  </h2>
                  ...
                  <div className="grid grid-cols-2 gap-8 w-full">
                    <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.04] space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-base font-bold text-white">{activeBniDeck === "eventos" ? "Serviços em Floripa" : activeBniDeck === "projetos" ? "O Escudo do Alvará" : activeBniDeck === "treinamentos" ? "Plano de Implantação (PIBI)" : "Conformidade Antecipada"}</h3>
                      </div>
                      <p className="text-xs text-gray-400 leading-relaxed">
                        {activeBniDeck === "eventos" && "O turismo de eventos é vital e move a economia da nossa capital de forma contínua."}
                        {activeBniDeck === "projetos" && "Possuir o AVCB ou Habite-se em dia blinda a sua empresa perante fiscalizações prediais de rotina."}
                        {activeBniDeck === "treinamentos" && "Sob a IN 28 do Corpo de Bombeiros (CBMSC), a regularidade predial exige a constituição da brigada orgânica corporativa."}
                        {activeBniDeck === "consultoria" && "Não espere a vistoria oficial surpresa de rotina dos oficiais bombeiros ou seguradoras prediais para consertar pendências."}
                      </p>
                      <div className="space-y-3 pt-2">
                        <div className="flex gap-4 items-center bg-white/[0.01] border border-white/[0.03] p-3 rounded-xl">
                          <span className="text-xl font-extrabold text-emerald-400">{activeBniDeck === "eventos" ? "76,1%" : activeBniDeck === "projetos" ? "100% Legal" : activeBniDeck === "treinamentos" ? "PIBI Exigido" : "Varredura"}</span>
                          <span className="text-[11px] text-gray-300 leading-relaxed">{activeBniDeck === "eventos" ? "Do PIB Municipal gerado pelo Setor de Serviços." : activeBniDeck === "projetos" ? "Total conformidade contra multas de fiscalização predial." : activeBniDeck === "treinamentos" ? "Obrigatório o protocolo digital no CBMSC para renovar atestado." : "Inspeção cirúrgica prévia em mais de 35 normas preventivas."}</span>
                        </div>
                        <div className="flex gap-4 items-center bg-white/[0.01] border border-white/[0.03] p-3 rounded-xl">
                          <span className="text-xl font-extrabold text-emerald-400">{activeBniDeck === "eventos" ? "R$ 100 Mi" : activeBniDeck === "projetos" ? "Seguro Ativo" : activeBniDeck === "treinamentos" ? "Brigadistas" : "Segurança"}</span>
                          <span className="text-[11px] text-gray-300 leading-relaxed">{activeBniDeck === "eventos" ? "Faturamento médio mensal de grandes eventos na cidade." : activeBniDeck === "projetos" ? "Garante total liquidez securitária em caso de sinistro." : activeBniDeck === "treinamentos" ? "Dimensione e prepare o número ideal de funcionários." : "Fim do estresse de multas ou lacração surpresa das portas."}</span>
                        </div>
                      </div>
                    </div>

                    <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.04] flex flex-col justify-center space-y-4 border-l-red-500/40 border-l-4">
              ...
              {currentSlideIndex === 2 && (
                <div className="w-full h-full flex flex-col justify-center space-y-8 relative">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-semibold max-w-fit uppercase tracking-wider">
                  ...
                  <div className="grid grid-cols-4 gap-4 w-full">
                    {[
                      { step: "Fase 1: Mapeamento", title: activeBniDeck === "eventos" ? "Briefing Técnico" : activeBniDeck === "projetos" ? "Vistoria Diagnóstica" : activeBniDeck === "treinamentos" ? "Análise de Riscos" : "Vistoria Prévia", desc: activeBniDeck === "eventos" ? "Estudo de lotação, rotas de fuga e vistorias de palcos." : activeBniDeck === "projetos" ? "Coleta de dados da edificação e layouts físicos locais." : activeBniDeck === "treinamentos" ? "Avaliação predial sob a IN 28 e dimensionamento." : "Inspeção dos hidrantes, SPDA, alarmes e rotas." },
                      { step: "Fase 2: Projeto", title: activeBniDeck === "eventos" ? "Documentação Ágil" : activeBniDeck === "projetos" ? "Projetos em CAD" : activeBniDeck === "treinamentos" ? "Conteúdo Programático" : "Roteiro Técnico", desc: activeBniDeck === "eventos" ? "Montagem técnica rápida de projetos e ARTs junto ao CBMSC." : activeBniDeck === "projetos" ? "Desenho técnico preventivo completo e dimensionamento de rotas." : activeBniDeck === "treinamentos" ? "Planejamento didático teórico-prático personalizado." : "Geração de Laudo Técnico detalhado sem rodeios." },
                      { step: "Fase 3: Protocolo", title: activeBniDeck === "eventos" ? "Dimensionamento" : activeBniDeck === "projetos" ? "Protocolo CBMSC" : activeBniDeck === "treinamentos" ? "Treinamento Realista" : "Trâmite Preventivo", desc: activeBniDeck === "eventos" ? "Escala precisa e legal de brigada de bombeiros civis." : activeBniDeck === "projetos" ? "Acompanhamento digital burocrático de análises do órgão." : activeBniDeck === "treinamentos" ? "Uso de fogo real e RCP massagem prática assistida." : "Varredura de conformidades de gás e extintores." },
                      { step: "Fase 4: Operação", title: activeBniDeck === "eventos" ? "Vistoria & Ação" : activeBniDeck === "projetos" ? "Vistoria & Alvará" : activeBniDeck === "treinamentos" ? "Homologação" : "Habite-se Concedido", desc: activeBniDeck === "eventos" ? "Brigada ativa de prontidão e vistoria rígida no local." : activeBniDeck === "projetos" ? "Acompanhamento de vistorias técnicas até alvará na mão." : activeBniDeck === "treinamentos" ? "Emissão de certificados na DAT/CBMSC de forma ágil." : "Entrega do atestado do CBMSC renovado sem multas." }
                    ].map((stepObj, sIdx) => (
                      ...
                      <div className="grid grid-cols-2 gap-8 w-full">
                        ...
                        <div className="relative rounded-2xl border border-white/[0.08] bg-[#06070a] h-[300px] overflow-hidden flex items-center justify-center group shadow-2xl">
                          ...
                          <div className="absolute inset-0 m-auto w-[90%] h-[80%] border border-dashed border-amber-500/15 rounded-xl flex items-center justify-center">
                            ...
                            <span className="text-[10px] text-amber-500/40 uppercase tracking-widest font-mono absolute bottom-4">CFD ACTIVE MODELING - PRESS P FOR NOTES</span>
                            {/* Scanning laser line */}
                        ...
                        <div className="cfd-response-display w-full max-h-[220px] self-center">
                          <div className="response-timeline">
                            <div className="response-node node-brigade">
                              ...
                              <span className="node-status text-green">Princípio de incêndio controlado na origem.</span>
                            </div>

                            <div className="response-node node-bombeiros">
                              <div className="flex justify-between items-center w-full">
                                <span className="node-label">Tempo Médio de Chegada dos Bombeiros Oficiais</span>
                                <span className="node-badge-time">10+ min (Caminhão)</span>
                              </div>
                              <div className="node-bar-container">
                                <div className="node-bar bar-red" style={{ width: "100%" }} />
                              </div>
                              <span className="node-status text-red">Grandes estragos estruturais e risco severo à integridade.</span>
                            </div>
                          </div>
                        </div>
                      ...
                      {activeBniDeck !== "consultoria" ? (
                        <div className="flex flex-col items-center space-y-4">
                          <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-white/20 bg-white/5 relative presenter-avatar-wrapper shadow-lg">
                            <img
                              src={activeBniDeck === "projetos" ? "/dione.png" : activeBniDeck === "treinamentos" ? "/murilo.png" : "/apresentador.png"}
                              alt="Apresentador"
                          ...
                          <div className="space-y-1 text-center">
                            <h4 className="font-bold text-sm text-white">
                              {activeBniDeck === "projetos" ? "Dione Borges" : activeBniDeck === "treinamentos" ? "Sargento BM Murilo Galdino" : "Paulo Roberto Ramos"}
                            </h4>
                            <p className="text-[10px] text-gray-400 leading-normal">
                              {activeBniDeck === "projetos" ? "Engenheira Civil & PPCI" : activeBniDeck === "treinamentos" ? "Especialista em Brigadas & Treinamentos" : "Especialista Operacional & Diretor"}
                            </p>
                          </div>
                        </div>
                      ) : (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {bniActiveNote ? (
                ...
                <p className="text-xs text-gray-500 italic">Selecione um slide para ver as anotações faladas.</p>
              )}
            </div>
    ...
    <div className={\`flex flex-col h-dvh overflow-hidden bg-background text-foreground select-none relative \${isPending ? "opacity-75 pointer-events-none" : ""}\`}>
      ...
      <header className="h-14 border-b border-border bg-card flex items-center justify-between px-6 z-20 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-2.5 px-3 py-1 rounded-lg bg-surface border border-border">
...
fs.writeFileSync(mainPagePath, unifiedCode, "utf-8");
console.log("Successfully unified presenting cockpits into", mainPagePath);
